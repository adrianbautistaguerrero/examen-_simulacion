...
class Parser:
    ...
    """
    def __init__(self):
        self.stemmer = None  # NLTK stemmer removido para simplificar
        self.mail = None
    ...
    def parse(self, raw_email):
        ...
        for line in lines:
            # Detectar inicio del cuerpo del email
            if not body_started:
                if line.strip() == '':
                    body_started = True
                continue
        ...
        # Limpiar texto
        body_text = re.sub(r'\s+', ' ', body_text)  # Normalizar espacios
        body_text = body_text.lower()  # Minúsculas
        body_text = re.sub(r'[^a-záéíóúñ\s]', '', body_text)  # Solo letras y espacios
