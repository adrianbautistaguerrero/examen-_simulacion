...
DATASET_PATH = os.path.join(BASE_DIR, 'trec07p')
...
def clean_path(original_path):
    ...
    # Crear ruta absoluta desde DATASET_PATH
...
def load_dataset(limit=None):
    ...
    Si limit es None, procesa todos los correos disponibles.
    """
    ...
    with open(index_path, 'r', errors='ignore') as f:
        ...
        for i, line in enumerate(lines):
            ...
            label = parts[0]  # 'spam' o 'ham'
            email_path = ' '.join(parts[1:])  # Ruta del correo
            
            # Limpiar y corregir la ruta
            ...
            try:
                with open(email_path, 'r', errors='ignore') as email_file:
                    ...
                    # Extraer Subject y Body (parsing básico)
                    subject = ''
                    body = content
                    ...
                    if processed_text.strip():  # Solo agregar si tiene contenido
...
def train_model():
    ...
    # Crear Pipeline: CountVectorizer + LogisticRegression
    ...
    pipeline = Pipeline([
        ('vectorizer', CountVectorizer(
            ...
            binary=True             # Usar presencia/ausencia en lugar de frecuencias
        )),
    ...
    # Guardar el modelo
    output_path = os.path.join(BASE_DIR, 'modelo_spam_final.joblib')
